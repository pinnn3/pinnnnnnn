                  ...
                  <div class="flex-grow">
                    ...
                    @if (job.status === 'failed' && job.error) {
                      <div class="flex items-center justify-between gap-4 mt-2">
                        <p class="text-red-400 text-sm">{{ job.error }}</p>
                        <button 
                            (click)="retryJob(job)"
                            class="px-4 py-1 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700 transition flex-shrink-0">
                            Retry
                        </button>
                      </div>
                    }
...
export class AppComponent {
  ...
  retryJob(jobToRetry: VideoJob): void {
    this.videoJobs.update(jobs =>
      jobs.map(j =>
        j.id === jobToRetry.id
          ? { ...j, status: 'pending', error: undefined, progressMessage: 'Queued for retry...' }
          : j
      )
    );
    this.processQueue();
  ...
  private async processQueue(): Promise<void> {
    ...
    const jobToProcess = this.videoJobs().find(j => j.status === 'pending');
  
    if (!jobToProcess) {
      this.isProcessing.set(false);
      this.isGenerating.set(false); // All jobs are done.
      return;
    }
    ...
    this.videoJobs.update(jobs => jobs.map(j =>
      j.id === jobToProcess.id
        ? { ...j, status: 'generating', progressMessage: this.getRandomProgressMessage() }
        : j
    ));
    ...
    try {
      progressInterval = setInterval(() => {
        this.videoJobs.update(jobs => jobs.map(j =>
          (j.id === jobToProcess.id && j.status === 'generating')
            ? { ...j, progressMessage: this.getRandomProgressMessage() }
            : j
        ));
      }, 8000);
  
      const { videoUrl, blob } = await this.geminiService.generateVideo(jobToProcess.prompt, this.apiKey(), this.shouldStop);
      clearInterval(progressInterval);
  
      this.videoJobs.update(jobs => jobs.map(j =>
        j.id === jobToProcess.id
          ? { ...j, status: 'completed', videoUrl, blob, progressMessage: 'Completed!' }
          : j
      ));
    } catch (e: any) {
      clearInterval(progressInterval);
      if (e.message === 'Video generation was stopped by the user.') {
        this.videoJobs.update(jobs => jobs.map(j =>
          j.id === jobToProcess.id
            ? { ...j, status: 'stopped', progressMessage: 'Cancelled by user.' }
            : j
        ));
      } else {
        this.videoJobs.update(jobs => jobs.map(j =>
          j.id === jobToProcess.id
            ? { ...j, status: 'failed', error: e.message, progressMessage: 'Failed!' }
            : j
        ));
      }
    } finally {
